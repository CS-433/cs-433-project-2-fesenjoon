...
def build_parser():
    ...
    parser.add_argument('--mlp-bias-checkpoint', type=str)
    parser.add_argument('--mlp-no-bias-checkpoint', type=str)
...
#                 real_parameter.mul_(args.checkpoint_shrink).add_(random_parameter, alpha=args.checkpoint_perturb)
    ...
    mlp_bias_model = models.get_model('mlp', bias=True, activation='relu', input_dim=32 * 32 * 3).to(device)
    mlp_no_bias_model = models.get_model('mlp', bias=False, activation='relu', input_dim=32 * 32 * 3).to(device)
    resnet18_model.load_state_dict(torch.load(args.resnet18_checkpoint, map_location=device)['model'])
    mlp_bias_model.load_state_dict(torch.load(args.mlp_bias_checkpoint, map_location=device)['model'])
    mlp_no_bias_model.load_state_dict(torch.load(args.mlp_no_bias_checkpoint, map_location=device)['model'])
    
    shrink_resnet18_model = models.get_model('resnet18', num_classes=10).to(device)
    shrink_mlp_bias_model = models.get_model('mlp', bias=True, activation='relu', input_dim=32 * 32 * 3).to(device)
    shrink_mlp_no_bias_model = models.get_model('mlp', bias=False, activation='relu', input_dim=32 * 32 * 3).to(device)
    ...
    resnet18_accuracies = []
    mlp_bias_accuracies = []
    mlp_no_bias_accuracies = []
    alphas = np.linspace(0, 1.0, 81)
    for alpha in tqdm(alphas):
        update_shrink_model(resnet18_model, shrink_resnet18_model, alpha)
        update_shrink_model(mlp_bias_model, shrink_mlp_bias_model, alpha)
        update_shrink_model(mlp_no_bias_model, shrink_mlp_no_bias_model, alpha)
        
        resnet18_accuracy = eval_on_dataloader(shrink_resnet18_model, loaders['test_loader'])
        mlp_bias_accuracy = eval_on_dataloader(shrink_mlp_bias_model, loaders['test_loader'])
        mlp_no_bias_accuracy = eval_on_dataloader(shrink_mlp_no_bias_model, loaders['test_loader'])
        resnet18_accuracies.append(resnet18_accuracy)
        mlp_bias_accuracies.append(mlp_bias_accuracy)
        mlp_no_bias_accuracies.append(mlp_no_bias_accuracy)
        
    import matplotlib.pyplot as plt
    plt.ylabel("Accuracy")
    plt.xlabel(r'$\lambda$')
    plt.plot(alphas, resnet18_accuracies, label='ResNet', color='C0')
    plt.plot(alphas, mlp_bias_accuracies, label='MLP with bias', color='C1')
    plt.plot(alphas, mlp_no_bias_accuracies, label='MLP without bias', color='C2')
    plt.legend()
